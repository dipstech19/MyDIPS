// =============================================================================
//  gestion_magasin.dart — VERSION RESPONSIVE CORRIGÉE (Desktop + Mobile)
//  Tous les overflows éliminés : Row/Column sécurisés, textes ellipsis,
//  largeurs fixes remplacées, layouts adaptatifs, formulaires sécurisés.
// =============================================================================
  ...
  factory Produit.fromFirestore(DocumentSnapshot doc) {
    ...
    return Produit(
      ...
      variantes: (d['variantes'] as List?)
          ?.map((v) =>
          VarianteProduit.fromMap(v as Map<String, dynamic>))
          .toList() ??
          [],
  ...
  factory Mouvement.fromFirestore(DocumentSnapshot doc) {
    ...
    return Mouvement(
      ...
      lignes: (d['lignes'] as List?)
          ?.map((l) =>
          LigneMouvement.fromMap(l as Map<String, dynamic>))
          .toList() ??
          [],
...
class MagasinProvider extends ChangeNotifier {
  ...
  Future<void> init() async {
    try {
      ...
      notifyListeners();
      final cp =
      _produitsRef.orderBy('nom').snapshots().listen((snap) {
      ...
      final cc =
      _categoriesRef.orderBy('nom').snapshots().listen((snap) {
      ...
      _cancelListeners.addAll([cp.cancel, cm.cancel, cc.cancel]);
    } catch (e) {
      _loading = false;
      _error = e.toString();
      _firebaseAvailable = false;
  ...
  Future<void> deleteProduit(String id) async =>
      _produitsRef.doc(id).delete();

  Future<void> _appliquerMouvement(Mouvement m,
      {required bool annuler}) async {
    ...
    if (!produit.aVariantes) {
      await prodDoc.update({
        'quantiteStock':
        (produit.quantiteStock + signe * m.quantite).clamp(0, 999999),
        'updatedAt': FieldValue.serverTimestamp()
      });
    } else {
      final newVariantes = produit.variantes.map((v) {
        final ligne = m.lignes.firstWhere((l) => l.unite == v.unite,
            orElse: () =>
                LigneMouvement(unite: v.unite, quantite: 0));
        return VarianteProduit(
            unite: v.unite,
            quantite:
            (v.quantite + signe * ligne.quantite).clamp(0, 999999));
      }).toList();
...
class GroupeUnites {
  ...
  const GroupeUnites(
      {required this.label, required this.emoji, required this.unites});
}
...
const List<GroupeUnites> kGroupes = [
  ...
  GroupeUnites(
      ...
      unites: [
        ...
        '5 kg', '10 kg', '25 kg', '50 kg', '100 kg', '1 T'
      ]),
  GroupeUnites(
      label: 'Pièces / Unités',
      emoji: '📦',
      unites: [
        'Unité', 'Boîte x10', 'Boîte x20', 'Carton x50', 'Palette'
      ]),
];

GroupeUnites? groupeByLabel(String? label) => label == null
    ? null
    : kGroupes.firstWhere((g) => g.label == label,
    orElse: () => kGroupes.first);
...
class _GestionMagasinPageState extends State<GestionMagasinPage>
  ...
  Widget build(BuildContext context) {
    ...
    if (magasin.loading)
      return const Scaffold(
          backgroundColor: kBg, body: Center(child: _LoadingState()));
        ...
        _TopNavBar(
          ...
          statChips: [
            if (_tab == 0) ...[
              ...
              if (rupt > 0)
                _StatChip(
                    '$rupt rupture${rupt > 1 ? "s" : ""}', kRedLt, kRed),
              if (bas > 0)
                _StatChip('$bas bas', kOrangeLt, kOrange),
            ],
            if (_tab == 1) ...[
              _StatChip(
                  '${magasin.entrees.length} entrée${magasin.entrees.length != 1 ? "s" : ""}',
                  kGreenLt,
                  kGreen),
            ...
            if (_tab == 2) ...[
              _StatChip(
                  '${magasin.sorties.length} sortie${magasin.sorties.length != 1 ? "s" : ""}',
                  kOrangeLt,
                  kOrange),
              _StatChip('$totalS unités', kBlueLt, kBlue),
            ],
            if (_tab == 3)
              _StatChip('$totalH opérations', kPurpleLt, kPurple),
          ],
        ),
        Expanded(
          child: TabBarView(
...
// ─────────────────────────────────────────────────────────────────────────────
//  SECTION 5 — TOP NAV BAR (responsive) — FIX overflow chips + tabs
// ─────────────────────────────────────────────────────────────────────────────
...
class _TopNavBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const tabs = [
      {
        'label': 'Stock',
        'labelFull': 'Stock Actuel',
        'icon': Icons.inventory_2_rounded,
        'color': kBlue
      },
      {
        'label': 'Entrées',
        'labelFull': 'Les Entrées',
        'icon': Icons.arrow_circle_down_rounded,
        'color': kGreen
      },
      {
        'label': 'Sorties',
        'labelFull': 'Les Sorties',
        'icon': Icons.arrow_circle_up_rounded,
        'color': kOrange
      },
      {
        'label': 'Historique',
        'labelFull': 'Historique',
    ...
    // FIX: tabs wrappés en Flexible dans leur Row parent
    final tabRow = Row(
              ...
              child: AnimatedContainer(
                ...
                child: Row(
                    ...
                    children: [
                      ...
                      // FIX: Flexible autour du texte pour éviter overflow
                      Flexible(
                        child: Text(label,
                            style: TextStyle(
                ...
                child: Row(children: [
                  ...
                  const Text('STOCK',
                      style: TextStyle(
                          ...
                          letterSpacing: .5)),
                  const Spacer(),
                  // FIX: chips dans un Flexible + scroll horizontal
          ...
          // Desktop
              ...
              child: Row(children: [
                ...
                if (statChips.isNotEmpty) ...[
                  ...
                  // FIX: chips en Flexible sur desktop aussi
                  Flexible(
                    child: SingleChildScrollView(
                      scrollDirection: Axis.horizontal,
                      child: Row(
...
// ─────────────────────────────────────────────────────────────────────────────
//  SECTION 6 — PAGE STOCK — FIX overflow dans table + cards
// ─────────────────────────────────────────────────────────────────────────────
...
class _StockPageState extends State<_StockPage> {
  ...
  List<Produit> get _list {
    ...
    var l = widget.produits.where((p) {
      ...
      return mq &&
          (_cat == 'Toutes' || p.categorie == _cat) &&
          (_mag == 'Tous' || p.magasin == _mag);
    }).toList();
    l.sort((a, b) {
      int c;
      if (_sort == 'ref')
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Padding(
        ...
        child: Row(children: [
          Text(
              '${list.length} / ${widget.magasin.totalProduits} produits',
      ...
      Expanded(
        child: list.isEmpty
            ? _EmptyState(
            message: 'Aucun produit trouvé', color: kBlue)
            : mobile
            ? ListView.separated(
          ...
          itemBuilder: (ctx, i) => _ProduitCard(
            ...
            onTap: () => _showDialog(
                ...
                _DetailsDialog(
                    produit: list[i],
                    groupe: groupeByLabel(
                        list[i].groupeUniteLabel))),
          ),
        )
            : Padding(
          padding: EdgeInsets.fromLTRB(
          ...
          child: _DataTable(
            ...
            rows: list.map((p) {
              ...
              final stockColor = p.rupture
                  ? kRed
                  : p.bas
                  ? kOrange
                  : kGreen;
              final stockBg = p.rupture
                  ? kRedLt
                  : p.bas
                  ? kOrangeLt
                  : kGreenLt;
              return _DataTableRow(cells: [
                // FIX: Row avec Flexible interne
                Row(children: [
                  Container(
                      width: 34,
                      height: 34,
                ...
                Center(
                    child: _IconBtn(
                        ...
                        kBlue, () {
                      _showDialog(context,
                          _DetailsDialog(produit: p, groupe: g));
                    })),
              ]);
            }).toList(),
          ),
...
// ── Card produit mobile — FIX overflow dans Row badges ────────────────────────
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: p.rupture ? kRed.withOpacity(0.3) : kBorder),
        boxShadow: [
          BoxShadow(
          ...
          child: Column(
              ...
              children: [
                          ...
                          child: Center(
                              child: Text(g?.emoji ?? '📦',
                                  style: const TextStyle(
                                      fontSize: 20)))),
                      const SizedBox(width: 10),
                      // FIX: Expanded pour que la colonne centrale ne déborde pas
                      Expanded(
                        child: Column(
                            crossAxisAlignment:
                            CrossAxisAlignment.start,
                            ...
                            children: [
                              ...
                              // FIX: Wrap au lieu de Row pour ref + badge
                              Wrap(
                                  spacing: 6,
                                  runSpacing: 4,
                                  children: [
                                    Text(p.reference,
                                        style: _mono.copyWith(
                                            fontSize: 11)),
                                    _PillBadge('Mag. ${p.magasin}',
                                        kIndigoLt, kIndigo),
                                  ]),
                            ]),
                      ),
                      const SizedBox(width: 8),
                      // FIX: Pas de Flexible autour du badge stock, il a sa taille fixe
                      Container(
                        padding: const EdgeInsets.symmetric(
                ...
                Row(children: [
                  _PillBadge(p.categorie, kBlueLt, kBlue),
                  const Spacer(),
                  // FIX: Wrap du texte + icon en Flexible
                  Flexible(
                    child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
...
class _EntreesPageState extends State<_EntreesPage> {
  ...
  List<Mouvement> get _list => widget.entrees
      .where((m) =>
  (_cat == 'Toutes' || m.categorie == _cat) &&
      (_mag == 'Tous' || m.magasin == _mag))
      .toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      _BanniereAction(
        ...
        sous:
        '${list.length} mouvement${list.length != 1 ? "s" : ""} · $total unités reçues',
        cats: cats,
        catVal: _cat,
        magVal: _mag,
        ...
        btnColor: kGreen,
        btnLabel: 'Nouvelle entrée',
        onBtnTap: () => _showDialog(
            context,
            _MouvForm(
                type: 'entree',
      ...
      Expanded(
        child: list.isEmpty
            ? _EmptyState(
            message:
            'Aucune entrée — cliquez sur « Nouvelle entrée »',
            color: kGreen)
            : mobile
            ? ListView.separated(
          padding: EdgeInsets.fromLTRB(
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      _BanniereAction(
        ...
        sous:
        '${list.length} mouvement${list.length != 1 ? "s" : ""} · $total unités sorties',
        cats: cats,
        catVal: _cat,
        magVal: _mag,
        onCatChanged: (v) => setState(() => _cat = v),
      ...
      Expanded(
        child: list.isEmpty
            ? _EmptyState(
            message:
            'Aucune sortie — cliquez sur « Nouvelle sortie »',
            color: kOrange)
            : mobile
            ? ListView.separated(
          padding: EdgeInsets.fromLTRB(
...
// ─────────────────────────────────────────────────────────────────────────────
//  CARD MOUVEMENT (mobile) — FIX overflow Row date+preneur+btn
// ─────────────────────────────────────────────────────────────────────────────
    ...
    child: Padding(
      ...
      child: Column(
          ...
          children: [
            ...
            // Ligne 2 : ref + catégorie + magasin — Wrap au lieu de Row
            Wrap(spacing: 5, runSpacing: 4, children: [
              Text(m.reference,
                  style: _mono.copyWith(fontSize: 11)),
              _PillBadge(m.categorie, kBlueLt, kBlue),
              _PillBadge('Mag. ${m.magasin}', kIndigoLt, kIndigo),
            ]),
            const SizedBox(height: 8),
            // Ligne 3 : date + preneur + supprimer
            // FIX: Row sécurisé avec Expanded sur preneur
            Row(children: [
              ...
              if (showPreneur && m.preneurNom != null) ...[
                ...
                const SizedBox(width: 3),
                // FIX: Expanded pour que le nom du preneur ne déborde pas
                Expanded(
                    child: Text(m.preneurNom!,
                        style: _muted.copyWith(fontSize: 11),
                        overflow: TextOverflow.ellipsis,
                        maxLines: 1)),
              ] else
                const Spacer(),
              const SizedBox(width: 6),
              _IconBtn(Icons.delete_outline_rounded, 'Supprimer',
                  kRedLt, kRed, onDelete),
            ]),
          ]),
    ),
  );
...
// ─────────────────────────────────────────────────────────────────────────────
//  SECTION 9 — BANNIÈRE ACTION — FIX overflow Row desktop
// ─────────────────────────────────────────────────────────────────────────────
...
class _BanniereAction extends StatelessWidget {
  ...
  const _BanniereAction({
    ...
    required this.magVal,
    required this.onCatChanged,
    required this.onMagChanged,
  ...
  Widget _iconTitre() => Row(children: [
    ...
    const SizedBox(width: 10),
    // FIX: Flexible sur la colonne texte
    Flexible(
      child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisSize: MainAxisSize.min,
  ...
  Widget build(BuildContext context) => Container(
    ...
    padding: EdgeInsets.symmetric(
        horizontal: 14, vertical: mobile ? 12 : 14),
    decoration: BoxDecoration(
          ...
          Row(children: [
            ...
            Expanded(
                child: _GlassDropdown(
                    ...
                    hint: 'Magasin',
                    onChanged: onMagChanged)),
          ]),
          const SizedBox(height: 8),
          SizedBox(width: double.infinity, child: _btn()),
        ])
    // FIX desktop: Row avec wrapping contrôlé
        : Row(children: [
      Expanded(child: _iconTitre()),
      const SizedBox(width: 12),
      // FIX: largeurs intrinsèques des dropdowns + bouton
      ConstrainedBox(
...
// ─────────────────────────────────────────────────────────────────────────────
//  SECTION 10 — PAGE HISTORIQUE — FIX overflow Row filtres desktop
// ─────────────────────────────────────────────────────────────────────────────
...
class _HistoriquePageState extends State<_HistoriquePage> {
  ...
  List<Mouvement> get _list {
    final all = [
      ...widget.magasin.entrees,
      ...widget.magasin.sorties
    ];
    ...
    return all.where((m) {
      ...
      final debutOk = _dateDebut == null ||
          !m.date.isBefore(DateTime(_dateDebut!.year,
              _dateDebut!.month, _dateDebut!.day));
      final finOk = _dateFin == null ||
          !m.date.isAfter(DateTime(_dateFin!.year, _dateFin!.month,
              _dateFin!.day, 23, 59, 59));
  ...
  Future<void> _pickDate(BuildContext context, bool isDebut) async {
    ...
    final picked = await showDatePicker(
      context: context,
      initialDate:
      isDebut ? (_dateDebut ?? now) : (_dateFin ?? now),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Container(
        ...
        child: Column(
            ...
            children: [
                    ...
                    Row(children: [
                      Container(
                          ...
                          child: const Icon(
                              ...
                              size: 18)),
                      const SizedBox(width: 10),
                      // FIX: Expanded sur la colonne titre
                      Expanded(
                        child: Column(
                            crossAxisAlignment:
                            CrossAxisAlignment.start,
              ...
              // FIX Desktop: Row avec Expanded sur titre + contraintes sur dropdowns
                  : Row(children: [
                Container(
                    padding: const EdgeInsets.all(9),
                    decoration: BoxDecoration(
              ...
              const SizedBox(height: 10),
              // Filtres date — Wrap est déjà correct
              Wrap(
                spacing: 6,
                runSpacing: 6,
      ...
      Expanded(
        child: list.isEmpty
            ? _EmptyState(
            message: 'Aucune opération pour ces filtres',
            color: kPurple)
            : mobile
            ? ListView.separated(
          padding: EdgeInsets.fromLTRB(
          ...
          itemBuilder: (ctx, i) {
            ...
            return _MouvCard(
              m: m,
              color: isE ? kGreen : kOrange,
              bgColor: isE ? kGreenLt : kOrangeLt,
              showPreneur: !isE,
              onDelete: () => _showDialog(
                  ctx,
                  _ConfirmDel(
...
class _MouvFormState extends State<_MouvForm> {
  ...
  GroupeUnites? get _groupe => _newProdMode
      ? groupeByLabel(_newGroupeLabel)
      : groupeByLabel(_selProd?.groupeUniteLabel);
  bool get _hasVar =>
      _newProdMode ? _newHasVar : (_selProd?.aVariantes ?? false);

  List<Produit> get _filteredProduits =>
      widget.magasin.produits.where((p) {
        return (_selMag == null || p.magasin == _selMag) &&
            (_selCat == null || p.categorie == _selCat);
      }).toList();
  ...
  void _validateStock() {
    ...
    if (!_hasVar) {
      ...
      setState(() => _stockError = demande > _selProd!.total
          ? 'Stock insuffisant (disponible : ${_selProd!.total})'
          : null);
    } else {
      final errors = <String, String>{};
      for (final u in _selVar) {
        final demande =
            int.tryParse(_varCtrl[u]?.text ?? '0') ?? 0;
        final varMatch =
        _selProd!.variantes.where((v) => v.unite == u);
        final dispo =
        varMatch.isEmpty ? 0 : varMatch.first.quantite;
  ...
  bool get _hasStockError => !_isSortie
      ? false
      : (!_hasVar
      ? _stockError != null
      : _varStockErrors.isNotEmpty);
  ...
  bool get _canSave {
    ...
    final catOk  = _newCatMode
        ? _newCatCtrl.text.trim().isNotEmpty
        : _selCat != null;
    final prodOk = _newProdMode
        ? (_newNomCtrl.text.trim().isNotEmpty &&
        _newRefCtrl.text.trim().isNotEmpty)
        : _selProd != null;
    final qteOk  = _hasVar
        ? _selVar.isNotEmpty
        : (int.tryParse(_qteC.text) ?? 0) > 0;
    final prenOk =
        !_isSortie || _preneurC.text.trim().isNotEmpty;
    return magOk &&
        catOk &&
        prodOk &&
        qteOk &&
    ...
    try {
      ...
      if (_newProdMode) {
        ...
        if (nom.isEmpty || ref.isEmpty)
          throw Exception('Nom ou référence manquant');
        if (_selMag == null)
        ...
        for (int i = 0; i < 40; i++) {
          if (widget.magasin.produits.any((p) => p.id == newId))
            break;
      ...
      if (!_hasVar && quantite <= 0)
        throw Exception('Quantité invalide');
      if (_hasVar && lignes.isEmpty)
        throw Exception('Sélectionnez au moins une variante');
      ...
      final mouvement = Mouvement(
        ...
        magasin: _selMag!,
        aVariantes: _hasVar,
        groupeUniteLabel: _hasVar
            ? (prod.groupeUniteLabel ?? _newGroupeLabel)
            : null,
        quantite: quantite,
        lignes: lignes,
        date: DateTime.now(),
        preneurNom: (_isSortie && _preneurC.text.trim().isNotEmpty)
            ? _preneurC.text.trim()
            : null,
      );
  ...
  Widget build(BuildContext context) {
    ...
    return _FullDialog(
      ...
      saveLabel:
      _saving ? 'Enregistrement…' : (_isSortie ? 'Valider la sortie' : "Valider l'entrée"),
      saving: _saving,
      child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // ── 1. Magasin ─────────────────────────────────────────────────
            _SectionHdr('1. Choisir le magasin',
                Icons.warehouse_rounded, _col),
            const SizedBox(height: 10),
            // FIX: GridView avec childAspectRatio adaptatif selon mobile/desktop
            LayoutBuilder(builder: (ctx, constraints) {
              final cols  = mobile ? 2 : 4;
              final ratio = mobile ? 1.5 : 1.3;
              return GridView.count(
                ...
                children: kMagasins.map((mag) {
                  ...
                  final prodCount = widget.magasin.produits
                      .where((p) => p.magasin == mag)
                      .length;
                  return InkWell(
            ...
            if (_selMag != null) ...[
              // ── 2. Catégorie ───────────────────────────────────────────────
              _SectionHdr(
                  '2. Catégorie', Icons.category_outlined, _col),
              const SizedBox(height: 10),
              // FIX: Row avec IntrinsicHeight pour aligner bouton et champ
                  ...
                  children: [
                    Expanded(
                        child: _newCatMode
                            ...
                            items: widget.magasin.categoryNames
              ...
              if (_selCat != null || _newCatMode) ...[
                // ── 3. Produit ─────────────────────────────────────────────
                _SectionHdr('3. Produit',
                    Icons.inventory_2_outlined, _col),
                const SizedBox(height: 10),
                Row(crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Expanded(
                ...
                if (_selProd != null || _newProdMode) ...[
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        const Icon(Icons.warning_amber_rounded,
                            color: kRed, size: 16),
                        const SizedBox(width: 8),
                        // FIX: Expanded sur le message d'erreur
                        Expanded(
                  ...
                  if (!_hasVar)
                  // FIX: width limité pour le champ quantité
                    ConstrainedBox(
                    ...
                    if (_selVar.isNotEmpty) ...[
                      ...
                      _VarQteTable(
                        ...
                        stockDisp: _isSortie && _selProd != null
                            ? {
                          for (var v in _selProd!.variantes)
                            v.unite: v.quantite
                        }
                            : {},
                ...
                if (_isSortie &&
                    (_selProd != null || _newProdMode)) ...[
                  _SectionHdr('5. Prélevé par',
                      Icons.person_outline_rounded, _col),
                  const SizedBox(height: 10),
                  _StyledTF(
                      ctrl: _preneurC,
                      hint: 'Nom et prénom…',
...
// ─────────────────────────────────────────────────────────────────────────────
//  SECTION 12 — WIDGETS FORMULAIRE — FIX overflows internes
// ─────────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Badge magasin — FIX: Row avec Flexible
            Container(
              padding: const EdgeInsets.symmetric(
              ...
              decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(7),
                  border:
                  Border.all(color: kIndigo.withOpacity(0.2))),
            ...
            // Nom + Ref — toujours en Column pour éviter overflow
            Column(children: [
              Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
            ...
            const SizedBox(height: 5),
            // FIX: Wrap au lieu de Row pour les boutons Oui/Non
            Wrap(spacing: 8, runSpacing: 8, children: [
              _ToggleBtn('Oui', hasVar, color,
                      () => onHasVarChanged(true)),
              _ToggleBtn('Non', !hasVar, color,
                      () => onHasVarChanged(false)),
            ]),
            if (hasVar) ...[
              const SizedBox(height: 12),
...
class _VarQteTable extends StatefulWidget {
  ...
  const _VarQteTable({
    required this.selVar,
    required this.varCtrl,
    required this.color,
    required this.onRemove,
    this.stockErrors = const {},
    this.stockDisp = const {},
...
class _VarQteTableState extends State<_VarQteTable> {
  ...
  Widget build(BuildContext context) {
    ...
    final total = widget.selVar.fold(
        0,
            (s, u) =>
        s + (int.tryParse(widget.varCtrl[u]?.text ?? '0') ?? 0));
    return Container(
      decoration: BoxDecoration(
          color: kSurface,
          borderRadius: BorderRadius.circular(kR),
      ...
      child: Column(children: [
        Container(
          ...
          child: Row(children: [
            // FIX: largeur fixe remplacée par Expanded avec flex
            Expanded(
                flex: 2,
                child: Text('UNITÉ',
                    style: _label.copyWith(color: col))),
            const SizedBox(width: 10),
            Expanded(
                flex: 5,
                child: Text('QUANTITÉ',
        ...
        ...widget.selVar.toList().asMap().entries.map((e) {
          ...
          return Column(children: [
            ...
            Padding(
              ...
              child: Column(children: [
                Row(children: [
                  // FIX: largeur fixe -> Flexible avec minWidth
                  ConstrainedBox(
                  ...
                  const SizedBox(width: 10),
                  // FIX: Expanded sur le bloc +/-/input
                  Expanded(
                    child: Row(children: [
                ...
                // Messages erreur/dispo — FIX: Row avec Expanded
                if (err != null)
                  Padding(
...
// ─────────────────────────────────────────────────────────────────────────────
//  SECTION 13 — DIALOGS — FIX _DetCard + variantes table
// ─────────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    ...
    return _FullDialog(
      ...
      child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // FIX: _DetCard prend toute la largeur sur mobile via Wrap
            Wrap(spacing: 8, runSpacing: 8, children: [
              _DetCard(Icons.qr_code_rounded, '🏷️ Référence',
                  p.reference),
              _DetCard(Icons.category_outlined, '📂 Catégorie',
                  p.categorie),
              _DetCard(Icons.warehouse_rounded, '🏪 Magasin',
                  'Magasin ${p.magasin}'),
              if (g != null)
                _DetCard(Icons.widgets_rounded,
                    '${g.emoji} Mesures', g.label),
            ]),
            const SizedBox(height: 14),
            // Stock total — FIX: Wrap au lieu de Row
            Container(
              padding: const EdgeInsets.symmetric(
            ...
            if (p.aVariantes && p.variantes.isNotEmpty) ...[
              ...
              Text(
                  'Détail par ${g?.label.toLowerCase() ?? "variante"}',
                  style: _h2),
              const SizedBox(height: 10),
              // FIX: toujours utiliser la vue liste (évite table avec colonnes trop étroites)
              Column(
                  children: p.variantes.map((v) {
                    ...
                    return Container(
                      ...
                      child: Row(children: [
                        // FIX: ConstrainedBox au lieu de largeur fixe
                        ConstrainedBox(
                          constraints: const BoxConstraints(
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                              color:
                              r ? kRedLt : lo ? kOrangeLt : kGreenLt,
                              borderRadius: BorderRadius.circular(20)),
                          child: Text(
                              r ? 'Rupture' : lo ? 'Bas' : 'OK',
                              style: TextStyle(
              ...
              Container(
                ...
                child: Row(
                    ...
                    children: [
                      Text('TOTAL',
                          style: _label.copyWith(color: kBlue)),
                      Text('${p.total}',
                          style: const TextStyle(
                              fontSize: 13,
                              fontWeight: FontWeight.w800,
  ...
  Widget build(BuildContext context) {
    ...
    return _FullDialog(
      ...
      child: Column(
          ...
          children: [
            Wrap(spacing: 8, runSpacing: 8, children: [
              _DetCard(Icons.inventory_2_outlined, '📦 Produit',
                  m.nomProduit),
              _DetCard(Icons.qr_code_rounded, '🏷️ Référence',
                  m.reference),
              _DetCard(Icons.category_outlined, '📂 Catégorie',
              ...
              _DetCard(Icons.warehouse_rounded, '🏪 Magasin',
                  'Magasin ${m.magasin}'),
              _DetCard(Icons.calendar_today_rounded, '📅 Date',
                  _fmtFull(m.date)),
              if (m.preneurNom != null)
                _DetCard(Icons.person_outline_rounded,
                    '👤 Prélevé par', m.preneurNom!),
            ]),
            const SizedBox(height: 14),
            // FIX: Wrap au lieu de Row pour le bandeau résumé
            Container(
              padding: const EdgeInsets.symmetric(
              ...
              child: Row(children: [
                ...
                Flexible(
                  child: Text(
                      isE
                          ? 'Entrée de stock — ${m.totalQte} unité${m.totalQte != 1 ? "s" : ""}'
                          : 'Sortie de stock — ${m.totalQte} unité${m.totalQte != 1 ? "s" : ""}',
                      style: TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.w700,
                          color: color),
            ...
            if (m.aVariantes && m.lignes.isNotEmpty) ...[
              ...
              Text(
                  ...
                  style: _h2),
              const SizedBox(height: 10),
              // FIX: liste au lieu de table avec colonnes fixes
              Column(
                  children: m.lignes.asMap().entries.map((e) {
                    final ligne = e.value;
                    return Container(
                      margin: const EdgeInsets.only(bottom: 6),
                      padding: const EdgeInsets.symmetric(
              ...
              Container(
                ...
                child: Row(
                    ...
                    children: [
                      Text('TOTAL',
                          style: _label.copyWith(color: color)),
                      Text('${m.totalQte}',
                          style: TextStyle(
                              fontSize: 13,
                              fontWeight: FontWeight.w800,
...
// ─────────────────────────────────────────────────────────────────────────────
//  SECTION 14 — FULL DIALOG SHELL — FIX contraintes taille + footer
// ─────────────────────────────────────────────────────────────────────────────
...
class _FullDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // FIX: marges et maxWidth adaptés pour éviter débordement lateral
      ...
      child: Material(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Container(
              padding: EdgeInsets.symmetric(
                  horizontal: mobile ? 16 : kP, vertical: 14),
              decoration: BoxDecoration(
                color: color,
                borderRadius: const BorderRadius.vertical(
                    top: Radius.circular(16)),
              ),
              // FIX: Row avec Flexible sur le titre
              child: Row(children: [
                Container(
            ...
            Flexible(
                child: SingleChildScrollView(
                    padding:
                    EdgeInsets.all(mobile ? 14 : kP),
                    child: child)),
            // ── Footer ───────────────────────────────────────────────────
            Container(
              padding: EdgeInsets.symmetric(
                  horizontal: mobile ? 14 : kP, vertical: 12),
              decoration: const BoxDecoration(
                  color: kBg,
            ...
            Container(
              ...
              child: Row(
                  children: columns
                      ...
                      flex: c.flex,
                      child: Text(c.label,
                          style: _label.copyWith(color: accent),
                          overflow: TextOverflow.ellipsis,
                          maxLines: 1)))
                      .toList()),
            ),
        ...
        children: List.generate(
            widget.columns.length,
              ...
              child: i < widget.row.cells.length
                  ...
                  // FIX: OverflowBox safe → clipper via overflow hidden
  ...
  List<Widget> get cells => [
    ...
    Center(
        child: _IconBtn(Icons.delete_outline_rounded,
            'Supprimer', kRedLt, kRed, onDelete)),
  ];
}
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
    child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.inbox_rounded,
            size: 52, color: color.withOpacity(0.2)),
        const SizedBox(height: 12),
        Text(message,
            style: _muted,
            textAlign: TextAlign.center),
...
// FIX _DetCard: largeur fixe → adaptative via LayoutBuilder
class _DetCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final mobile = isMobile(context);
    final sw     = MediaQuery.of(context).size.width;
    // FIX: sur mobile pleine largeur, sur desktop ~175 ou adapté
        ...
        Flexible(
          child: Column(
              ...
              children: [
                ...
                Text(val,
                    style: _h2.copyWith(fontSize: 11),
                    overflow: TextOverflow.ellipsis,
                    maxLines: 1),
              ]),
        ),
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(8),
        border: Border.all(
            color: active ? color : kBorder, width: 1.5),
      ),
      child: Text(label,
          style: TextStyle(
              fontSize: 12,
...
class _SectionHdr extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    const SizedBox(width: 8),
    // FIX: Flexible sur le texte du header section
    Flexible(
...
class _TypeToggleRow extends StatelessWidget {
  ...
  const _TypeToggleRow(
      {required this.typeFiltre, required this.onChanged});
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: Colors.white
                .withOpacity(date != null ? 0.6 : 0.3),
            width: date != null ? 1.5 : 1),
      ),
      // FIX: Wrap au lieu de Row pour éviter overflow sur petit écran
      child: Wrap(
          spacing: 4,
          crossAxisAlignment: WrapCrossAlignment.center,
          children: [
  ...
  Widget build(BuildContext context) =>
      SizedBox(height: 42, child: TextField(
        ...
        decoration: InputDecoration(
          ...
          hintStyle:
          const TextStyle(fontSize: 12, color: kBorderMd),
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
          color: kBlueLt,
          borderRadius: BorderRadius.circular(kR),
          border: Border.all(color: kBlueMd.withOpacity(.5))),
      child: Icon(
          asc
...
class _LoadingState extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 12),
        const Text('Chargement du stock…',
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
              ...
              child: const Icon(Icons.cloud_off_rounded,
                  color: kRed, size: 32)),
          const SizedBox(height: 16),
          const Text('Firebase non disponible',
              style: TextStyle(
                  fontSize: 18,
