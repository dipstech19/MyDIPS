...
class _GestionMagasinFirebaseState extends State<GestionMagasinFirebase>
  ...
  Widget build(BuildContext context) {
    ...
    if (magasin.loading) return const Center(child: CircularProgressIndicator());
        ...
        child: Padding(
          ...
          // ════════════════════════════════
            ...
            children: [
              ...
              // Ligne 2 : onglets pleine largeur
              Padding(
          ...
          // ════════════════════════════════
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 16),
              // Onglets pleine largeur restante
...
class _StatChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(20), border: Border.all(color: col.withOpacity(0.25))),
    ...
    return Column(children: [
      Padding(
        ...
        child: mobile
          ...
          const SizedBox(height: 8),
          Row(children: [
            Expanded(child: _DropBox(value: _sort, items: ['nom','ref','stock','mag'],
          ...
          child: _DataTable(
            ...
            rows: list.map((m) => _MouvRow(
              ...
              onDelete: () => _showDialog(context, _ConfirmDel(
                ...
                onConfirm: () { widget.magasin.deleteEntree(m.id); Navigator.of(context, rootNavigator: true).pop(); },
              )),
            )).toList(),
          ),
        ),
          ...
          child: _DataTable(
            ...
            rows: list.map((m) => _MouvRow(
              ...
              onDelete: () => _showDialog(context, _ConfirmDel(
                ...
                onConfirm: () { widget.magasin.deleteSortie(m.id); Navigator.of(context, rootNavigator: true).pop(); },
              )),
            )).toList(),
          ),
        ),
...
// ══════════════════════════════════════════════════════════════
//  BANNIÈRE ACTION  (réutilisable Entrées / Sorties)
// ══════════════════════════════════════════════════════════════
class _BanniereAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final box = Container(
      ...
      child: mobile ? _mobileLayout(context) : _desktopLayout(context),
    );
    return box;
  }
  ...
  Widget _mobileLayout(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    _iconTitre(),
    const SizedBox(height: 12),
    Row(children: [
...
class _HistoriquePage extends StatefulWidget {
  ...
  @override State<_HistoriquePage> createState() => _HistoriquePageState();
}
...
class _HistoriquePageState extends State<_HistoriquePage> {
    ...
    final picked = await showDatePicker(
      ...
      builder: (ctx, child) => Theme(
        data: Theme.of(ctx).copyWith(
          colorScheme: const ColorScheme.light(primary: kPurple, onPrimary: Colors.white, surface: kSurface),
        ),
        child: child!,
      ),
    );
  ...
  void _resetDates() => setState(() { _dateDebut = null; _dateFin = null; });
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Container(
        ...
        child: mobile
            ? _HistoBanniereMobile(
          list: list, typeFiltre: _typeFiltre, mag: _mag,
          dateDebut: _dateDebut, dateFin: _dateFin,
          onTypeChanged: (v) => setState(() => _typeFiltre = v),
          onMagChanged:  (v) => setState(() => _mag = v),
          onPickDebut:   () => _pickDate(context, true),
          onPickFin:     () => _pickDate(context, false),
          onReset:       _resetDates,
        )
            : _HistoBannierDesktop(
          list: list, typeFiltre: _typeFiltre, mag: _mag,
          dateDebut: _dateDebut, dateFin: _dateFin,
          onTypeChanged: (v) => setState(() => _typeFiltre = v),
          onMagChanged:  (v) => setState(() => _mag = v),
            ...
            rows: list.map((m) {
              ...
              return _DataTableRow(cells: [
                ...
                m.preneurNom != null
                  ...
                  Flexible(child: Text(m.preneurNom!, style: _body.copyWith(fontSize: 11), overflow: TextOverflow.ellipsis, maxLines: 1)),
                ])
                    : Text('—', style: _muted),
...
// ── Bannière Historique Desktop ──────────────────────────────────
class _HistoBannierDesktop extends StatelessWidget {
  final List<Mouvement> list;
  final String typeFiltre, mag;
  final DateTime? dateDebut, dateFin;
  final ValueChanged<String> onTypeChanged, onMagChanged;
  final VoidCallback onPickDebut, onPickFin, onReset;

  const _HistoBannierDesktop({
    required this.list, required this.typeFiltre, required this.mag,
    this.dateDebut, this.dateFin,
    required this.onTypeChanged, required this.onMagChanged,
    required this.onPickDebut, required this.onPickFin, required this.onReset,
  });

  @override
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    // Ligne 1 : titre + boutons type + magasin
    Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
      ...
      const SizedBox(width: 12),
      _TypeToggleRow(typeFiltre: typeFiltre, onChanged: onTypeChanged),
      const SizedBox(width: 12),
      _GlassDropdown(value: mag, items: ['Tous', ...kMagasins],
          labels: {'Tous': 'Tous magasins', ...{for (var m in kMagasins) m: 'Mag. $m'}},
          icon: Icons.warehouse_rounded, hint: 'Magasin', onChanged: onMagChanged),
    ]),
    const SizedBox(height: 12),
    // Ligne 2 : filtre plage de dates
    Row(children: [
      ...
      const SizedBox(width: 12),
      _DatePickerBtn(label: 'Du', date: dateDebut, onTap: onPickDebut),
      const SizedBox(width: 8),
      _DatePickerBtn(label: 'Au', date: dateFin,   onTap: onPickFin),
      if (dateDebut != null || dateFin != null) ...[
        const SizedBox(width: 8),
        InkWell(onTap: onReset, borderRadius: BorderRadius.circular(7),
...
// ── Bannière Historique Mobile ────────────────────────────────────
class _HistoBanniereMobile extends StatelessWidget {
  final List<Mouvement> list;
  final String typeFiltre, mag;
  final DateTime? dateDebut, dateFin;
  final ValueChanged<String> onTypeChanged, onMagChanged;
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [
      ...
      const SizedBox(width: 10),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        const Text('Historique', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Colors.white)),
        Text('${list.length} opération${list.length != 1 ? "s" : ""}',
            style: TextStyle(fontSize: 11, color: Colors.white.withOpacity(0.75))),
      ])),
    ]),
    ...
    const SizedBox(height: 10),
    Row(children: [
      const Icon(Icons.date_range_rounded, color: Colors.white70, size: 15),
      const SizedBox(width: 6),
      const Text('Période :', style: TextStyle(fontSize: 11, color: Colors.white70, fontWeight: FontWeight.w600)),
      const SizedBox(width: 8),
      Expanded(child: _DatePickerBtn(label: 'Du', date: dateDebut, onTap: onPickDebut)),
      const SizedBox(width: 6),
      Expanded(child: _DatePickerBtn(label: 'Au', date: dateFin,   onTap: onPickFin)),
      if (dateDebut != null || dateFin != null) ...[
        const SizedBox(width: 6),
        InkWell(onTap: onReset, borderRadius: BorderRadius.circular(7),
...
class _TypeToggleRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Row(mainAxisSize: MainAxisSize.min, children: ['Tout','Entrées','Sorties'].map((t) {
...
class _MouvForm extends StatefulWidget {
  final String type;
  final MagasinProvider magasin;
  final BuildContext? scaffoldContext; // contexte parent pour SnackBar
...
class _MouvFormState extends State<_MouvForm> {
  ...
  void _validateStockSortie() {
    if (!_isSortie || _selProd == null) { setState(() { _stockError = null; _varStockErrors = {}; }); return; }
    if (!_hasVar) {
  ...
  bool get _hasStockError {
    if (!_isSortie) return false;
    return !_hasVar ? _stockError != null : _varStockErrors.isNotEmpty;
  }
  ...
  Future<void> _save() async {
    ...
    // Capturer les contextes AVANT tout await
    ...
    try {
      ...
      if (_newProdMode) {
        ...
        if (newId.isEmpty) throw Exception("Erreur création produit (id vide)");
        // On reconstruit le produit avec son vrai id
      ...
      // ── Si nouveau produit, attendre qu'il apparaisse dans le stream ──────
      if (_newProdMode && prod.id.isNotEmpty) {
        for (int i = 0; i < 40; i++) {
          if (widget.magasin.produits.any((p) => p.id == prod.id)) break;
          ...
          if (_selCat != null || _newCatMode) ...[
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(child: _newProdMode
                  ...
                  onChanged: (v) => setState(() { _selProd = v; _selVar.clear(); _varCtrl.clear(); _validateStockSortie(); }))),
              const SizedBox(width: 8),
              _ModeBtn(label: _newProdMode ? '← Existant' : '+ Nouveau', color: _col,
                  onTap: () => setState(() { _newProdMode = !_newProdMode; _selProd = null; _selVar.clear(); _varCtrl.clear(); _stockError = null; _varStockErrors = {}; })),
            ]),
            const SizedBox(height: 22),
            ...
            if (_selProd != null || _newProdMode) ...[
              ...
              if (!_hasVar)
                SizedBox(width: 180, child: _NumStepField(ctrl: _qteC, color: _col, onChanged: () { setState(() {}); _validateStockSortie(); }))
                  ...
                  return InkWell(
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        if (disp != null)
                          Text('$disp dispo', style: TextStyle(fontSize: 9, color: sel ? Colors.white70 : kMuted)),
                      ]),
                    ),
                  );
                }).toList()),
                if (_selVar.isNotEmpty) ...[
                  ...
                  _VarQteTable(
                    ...
                    stockDisp: _isSortie && _selProd != null
                        ...
                        : {},
                    onQteChanged: _validateStockSortie,
                  ),
                ],
              ],
              const SizedBox(height: 22),
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          const Icon(Icons.warehouse_rounded, size: 13, color: kIndigo), const SizedBox(width: 6),
          Flexible(child: Text('Enregistré dans Magasin $mag', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: kIndigo))),
        ]),
      ),
      const SizedBox(height: 12),
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('NOM DU PRODUIT', style: _label), const SizedBox(height: 5),
          _StyledTF(ctrl: nomCtrl, hint: 'Ex: Casque de sécurité', onChanged: (_) => onChanged()),
        ])),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('RÉFÉRENCE', style: _label), const SizedBox(height: 5),
          _StyledTF(ctrl: refCtrl, hint: 'Ex: EPI-010', onChanged: (_) => onChanged()),
        ])),
      ]),
      const SizedBox(height: 12),
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('A DES VARIANTES ?', style: _label), const SizedBox(height: 5),
        Row(children: [
      ...
      if (hasVar) ...[
        const SizedBox(height: 12),
        const Text("GROUPE D'UNITÉS", style: _label), const SizedBox(height: 5),
        _StyledDrop<String>(
                ...
                if (err != null)
                  Padding(padding: const EdgeInsets.only(top: 4),
                      child: Row(children: [
                        ...
                        const Icon(Icons.error_outline, size: 12, color: kRed), const SizedBox(width: 3),
                  ...
                  Padding(padding: const EdgeInsets.only(top: 3),
                      child: Row(children: [
                        const SizedBox(width: 110),
                        Icon(Icons.info_outline, size: 11, color: kMuted.withOpacity(0.7)), const SizedBox(width: 3),
              ...
              ...p.variantes.asMap().entries.map((e) {
                ...
                return Column(children: [
                  ...
                  Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), child: Row(children: [
                    ...
                    Expanded(flex: 1, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                        decoration: BoxDecoration(color: vb, borderRadius: BorderRadius.circular(20)),
                        child: Text(r ? 'Rupture' : lo ? 'Bas' : 'OK',
                            textAlign: TextAlign.center, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: vc)))),
                  ])),
                ]);
              }),
              Container(color: kBlueLt, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 9),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            Flexible(child: Text(isE ? 'Entrée de stock — ${m.totalQte} unité${m.totalQte!=1?"s":""}' : 'Sortie de stock — ${m.totalQte} unité${m.totalQte!=1?"s":""}',
        ...
        if (m.aVariantes && m.lignes.isNotEmpty) ...[
          ...
          Container(
            ...
            decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(kR), border: Border.all(color: color.withOpacity(0.2))),
            child: Row(children: [
              Icon(Icons.tag_rounded, color: color, size: 16), const SizedBox(width: 8),
...
class _DetCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: kBg, borderRadius: BorderRadius.circular(kR), border: Border.all(color: kBorder)),
    child: Row(children: [
      Icon(icon, size: 14, color: kBlue), const SizedBox(width: 8),
      Flexible(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
...
class _ConfirmDel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _FullDialog(
    ...
    child: Container(
      ...
      child: Row(children: [
        const Icon(Icons.warning_amber_rounded, color: kRed, size: 20), const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              child: Row(children: [
                ...
                InkWell(onTap: () => Navigator.of(context, rootNavigator: true).pop(), borderRadius: BorderRadius.circular(7),
                    child: Container(padding: const EdgeInsets.all(6),
                        decoration: BoxDecoration(color: Colors.white.withOpacity(0.18), borderRadius: BorderRadius.circular(7)),
...
// ══════════════════════════════════════════════════════════════
//  LIGNE MOUVEMENT  (quantité totale uniquement)
// ══════════════════════════════════════════════════════════════
class _MouvRow extends _DataTableRow {
  ...
  List<Widget> get cells => [
    ...
    if (showPreneur)
      Row(mainAxisSize: MainAxisSize.min, children: [
        const Icon(Icons.person_outline_rounded, size: 12, color: kMuted), const SizedBox(width: 4),
        Flexible(child: Text(m.preneurNom ?? '—', style: _body.copyWith(fontSize: 11), overflow: TextOverflow.ellipsis, maxLines: 1)),
      ]),
    Row(mainAxisSize: MainAxisSize.min, children: [
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: ClipRRect(borderRadius: BorderRadius.circular(kR2), child: Column(children: [
        Container(color: accent.withOpacity(0.07), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
    ...
    child: AnimatedContainer(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.center,
          children: List.generate(widget.columns.length, (i) =>
              Expanded(flex: widget.columns[i].flex,
                  child: i < widget.row.cells.length
                      ? OverflowBox(maxWidth: double.infinity, child: Align(alignment: Alignment.centerLeft, child: widget.row.cells[i]))
...
class _SearchBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(height: 42, child: TextField(
      ...
      decoration: InputDecoration(
          hintText: 'Rechercher un produit…', hintStyle: const TextStyle(fontSize: 12, color: kBorderMd),
...
class _DropBox extends StatelessWidget {
  final dynamic value; final List<String> items; final Map<String, String>? labels; final void Function(String) onChanged;
...
class _StyledDrop<T> extends StatelessWidget {
  final T? value; final String? hint; final List<DropdownMenuItem<T>> items; final void Function(T?) onChanged;
...
class _StyledTF extends StatelessWidget {
  final TextEditingController ctrl; final String hint; final Widget? prefix; final void Function(String)? onChanged;
  const _StyledTF({required this.ctrl, required this.hint, this.prefix, this.onChanged});
  @override
  Widget build(BuildContext context) => TextField(
      controller: ctrl, style: const TextStyle(fontSize: 12, color: kText), onChanged: onChanged,
      decoration: InputDecoration(
          hintText: hint, hintStyle: const TextStyle(fontSize: 12, color: kBorderMd), prefixIcon: prefix,
...
class _StepBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => InkWell(borderRadius: BorderRadius.circular(7), onTap: onTap,
      child: Container(width: 44, height: 44,
          decoration: BoxDecoration(color: color.withOpacity(.09), borderRadius: BorderRadius.circular(7), border: Border.all(color: color.withOpacity(.2))),
