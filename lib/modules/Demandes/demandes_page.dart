...
class _Equipe {
  ...
  const _Equipe({
    required this.id, required this.nom, required this.chef, required this.employes,
  });
}
...
// ─── Modèle ───────────────────────────────────────────────────────────────────
class _Demande {
  final String type, equipe, employeNom, employePoste;
  final Map<String, String> extras;
  String statut;

  _Demande({
    required this.type, required this.equipe,
    required this.employeNom, required this.employePoste,
    required this.extras, this.statut = 'En attente',
  });
}
...
class DemandesPage extends StatefulWidget {
  ...
  @override State<DemandesPage> createState() => _DemandesPageState();
}
...
class _DemandesPageState extends State<DemandesPage> {
  ...
  // Labels raccourcis pour mobile
  final List<String> _tabsMobile = [
    'Congé',
    'Att. travail',
    'Bulletin',
    'Att. salaire',
  ...
  void _update(_Demande old, _Demande neo) {
    final i = _demandes.indexOf(old);
    if (i >= 0) setState(() => _demandes[i] = neo);
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            color: kSurface,
            padding: EdgeInsets.fromLTRB(padding, topPad + (mobile ? 14 : padding), padding, 14),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              ...
              children: [
                        ...
                        children: [
                          Text('Demandes',
                              style: TextStyle(
                                  ...
                                  color: kText,
                                  letterSpacing: -0.4)),
                          Text('Gestion des demandes administratives',
                              style: TextStyle(fontSize: mobile ? 11 : 12, color: kSubtext)),
                        ],
                      ),
                    ),
                  ],
                ...
                // Pill tabs — labels courts sur mobile
                      ...
                      return GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                                color: sel ? kBlue : const Color(0xFFDDE4ED), width: 1.5),
                          ),
                          child: Text(label,
                              style: TextStyle(
...
class _TabBody extends StatefulWidget {
  ...
  const _TabBody({
    super.key, required this.tab, required this.demandes,
    required this.onAdd, required this.onUpdate, required this.onDelete,
  });

  @override State<_TabBody> createState() => _TabBodyState();
}
        ...
        children: [
          ...
          AnimatedSwitcher(
            ...
            child: _showForm
                ...
                type: _type, editing: _editing,
                onSaved: _onSaved, onCancel: _closeForm)
          ...
          _DCard(
            ...
            child: widget.demandes.isEmpty
                ...
                : Column(
              mainAxisSize: MainAxisSize.min,
              children: widget.demandes.map((d) => _DemandeRow(
                demande: d, canEdit: _canEdit,
                onEdit: () => _openEdit(d),
...
class _DemandeForm extends StatefulWidget {
  ...
  const _DemandeForm({
    super.key, required this.type, required this.editing,
    required this.onSaved, required this.onCancel,
  });

  @override State<_DemandeForm> createState() => _DemandeFormState();
}
...
class _DemandeFormState extends State<_DemandeForm>
  ...
  Map<String, String> _buildExtras() {
    switch (widget.type) {
      ...
      case 'bulletin':    return {'mois': _moisCtrl.text, 'salaire': '${_salaireCtrl.text} DH'};
      case 'attest_sal':  return {'salaire': '${_salaireCtrl.text} DH', 'motif': _motifCtrl.text};
      default:            return {};
    }
  }
  ...
  Future<void> _submit() async {
    ...
    widget.onSaved(_Demande(
      type: widget.type, equipe: _equipe!.nom,
      employeNom: _employe!.nom, employePoste: _employe!.poste,
      extras: _buildExtras(),
    ));
  }
    ...
    return ScaleTransition(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: kBlue.withOpacity(0.07), blurRadius: 28, offset: const Offset(0, 8))],
        ),
        child: Form(
          key: _fk,
          child: Column(
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Dropdowns : colonne sur mobile, ligne sur desktop
                    if (mobile) ...[
                      _DropField<_Equipe>(
  ...
  // Champs spécifiques — colonne sur mobile, ligne sur desktop
        ...
        if (mobile) {
          return [
            ...
            _TField(ctrl: _dureeCtrl, label: 'Durée (jours)',
                icon: Icons.hourglass_bottom_outlined, type: TextInputType.number, req: true),
          ];
        }
        return [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (mobile) {
          return [
            _TField(ctrl: _salaireCtrl, label: 'Salaire (DH)',
                icon: Icons.payments_outlined, type: TextInputType.number, req: true),
            const SizedBox(height: 10),
            _TField(ctrl: _motifCtrl, label: 'Motif', icon: Icons.info_outline, req: true),
          ];
        }
        return [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
// ─────────────────────────────────────────────────────────────────────────────
//  LIGNE DEMANDE — responsive (carte sur mobile)
// ─────────────────────────────────────────────────────────────────────────────
class _DemandeRow extends StatefulWidget {
  ...
  const _DemandeRow({
    required this.demande, required this.canEdit,
    required this.onEdit, required this.onDelete,
  });

  @override State<_DemandeRow> createState() => _DemandeRowState();
}
...
class _DemandeRowState extends State<_DemandeRow>
  ...
  Future<void> _telechargerPdf(BuildContext context, _Demande d) async {
    ...
    try {
      ...
      final dateDoc = '${now2.day.toString().padLeft(2, '0')}/'
      ...
      switch (d.type) {
        case 'conge':
          final bytes = await DipsPdfService.genererDemandeConge(
            logoBytes: logoBytes, nomEmploye: d.employeNom,
            telephone: d.extras['telephone'] ?? '',
            nbJours: int.tryParse((d.extras['duree'] ?? '0').replaceAll(' jours', '')) ?? 0,
        ...
        case 'attest_trav':
          final bytes = await DipsPdfService.genererAttestationTravail(
            logoBytes: logoBytes, nomEmploye: d.employeNom, poste: d.employePoste,
            dateEmbauche: d.extras['dateEmbauche'] ?? '',
            dateDocument: dateDoc, motif: d.extras['motif'] ?? '',
          );
        ...
        case 'bulletin':
          ...
          final bytes = await DipsPdfService.genererBulletinPaie(
            logoBytes: logoBytes, nomEmploye: d.employeNom, poste: d.employePoste,
            mois: d.extras['mois'] ?? '', salaireBrut: sal,
            cnss: sal * 0.0448, amo: sal * 0.0226, ir: sal * 0.10,
            dateDocument: dateDoc,
          );
          await DipsPdfService.imprimer(bytes, 'Bulletin_${d.employeNom}_${d.extras["mois"] ?? ""}');
          break;
        ...
        case 'attest_sal':
          ...
          final bytes = await DipsPdfService.genererAttestationSalaire(
            logoBytes: logoBytes, nomEmploye: d.employeNom, poste: d.employePoste,
            salaireMensuel: sal, motif: d.extras['motif'] ?? '',
            dateDocument: dateDoc,
          );
  ...
  Widget build(BuildContext context) {
    ...
    return FadeTransition(
      opacity: _fade,
      child: SlideTransition(
        position: Tween<Offset>(begin: const Offset(0.02, 0), end: Offset.zero).animate(_fade),
        child: Container(
          margin: const EdgeInsets.only(bottom: 10),
          padding: EdgeInsets.symmetric(
              horizontal: mobile ? 12 : 14,
              vertical: mobile ? 12 : 12),
          decoration: BoxDecoration(
            color: kSurface, borderRadius: BorderRadius.circular(14),
            border: Border.all(color: const Color(0xFFE8EFF7)),
            boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.02), blurRadius: 6, offset: const Offset(0, 2))],
          ),
          // Sur mobile : layout en colonne (carte), sur desktop : row classique
          child: mobile ? _buildMobile(d) : _buildDesktop(d),
        ),
      ),
    );
  }
  ...
  // ── Layout desktop (row) ───────────────────────────────────────────────────
  Widget _buildDesktop(_Demande d) => Row(
    crossAxisAlignment: CrossAxisAlignment.center,
    children: [
      ...
      Expanded(child: Column(
        ...
        children: [
          Row(children: [
            Flexible(child: Text(d.employeNom,
                style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13.5, color: kText),
                overflow: TextOverflow.ellipsis)),
            const SizedBox(width: 6),
            _TeamChip(label: d.equipe),
          ]),
          const SizedBox(height: 3),
          Text(_subtitle(), style: const TextStyle(fontSize: 11.5, color: kSubtext),
              overflow: TextOverflow.ellipsis),
        ],
      )),
      const SizedBox(width: 10),
  ...
  // ── Layout mobile (carte) ──────────────────────────────────────────────────
  Widget _buildMobile(_Demande d) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    mainAxisSize: MainAxisSize.min,
    children: [
      // Ligne 1 : avatar + nom + équipe
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _Avatar(d.employeNom),
        const SizedBox(width: 10),
        Expanded(child: Column(
  ...
  Widget _buildActions() => Row(
    ...
    children: [
      if (_approved)
        _IBtn(
          icon: Icons.download_rounded, color: kBlue,
          tooltip: 'Télécharger PDF',
          onTap: () => _telechargerPdf(context, widget.demande),
        ),
      if (widget.canEdit)
        _IBtn(
          icon: Icons.edit_outlined, color: const Color(0xFF64748B),
...
class _ToastWidgetState extends State<_ToastWidget>
  ...
  void initState() {
    ...
    _ctrl.forward();
    Future.delayed(const Duration(milliseconds: 2100), () { if (mounted) _ctrl.reverse(); });
  }
    ...
    child: AnimatedBuilder(
      ...
      child: Center(child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(100),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.16), blurRadius: 20, offset: const Offset(0, 5))],
        ),
  ...
  return showGeneralDialog<bool>(
    ...
    pageBuilder: (ctx, _, __) => Center(child: Material(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: kSurface, borderRadius: BorderRadius.circular(20),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.12), blurRadius: 36, offset: const Offset(0, 12))],
        ),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 8),
          Text(message, style: const TextStyle(fontSize: 12.5, color: kSubtext, height: 1.5),
              textAlign: TextAlign.center),
          const SizedBox(height: 20),
          Row(children: [
...
class _DCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    decoration: BoxDecoration(
      ...
      boxShadow: [BoxShadow(color: kBlue.withOpacity(0.04), blurRadius: 20, offset: const Offset(0, 5))],
    ),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
      Padding(
        padding: const EdgeInsets.fromLTRB(16, 14, 16, 0),
        child: Text(title, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: kSubtext, letterSpacing: 0.5)),
      ),
      const SizedBox(height: 8),
      Container(height: 1, margin: const EdgeInsets.symmetric(horizontal: 16), color: const Color(0xFFF0F4F8)),
...
class _SectionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(mainAxisSize: MainAxisSize.min, children: [
    Container(width: 3, height: 13, decoration: BoxDecoration(color: kBlue, borderRadius: BorderRadius.circular(2))),
    const SizedBox(width: 8),
    Text(label, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: kSubtext, letterSpacing: 0.8)),
  ]);
}
...
class _DropField<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) => DropdownButtonFormField<T>(
    ...
    decoration: InputDecoration(
      labelText: label,
      labelStyle: TextStyle(fontSize: 12.5, color: enabled ? kSubtext : kSubtext.withOpacity(0.5)),
      prefixIcon: Icon(icon, size: 17, color: enabled ? kBlue : kSubtext.withOpacity(0.4)),
      filled: true, fillColor: enabled ? kBg : kBg.withOpacity(0.6),
      contentPadding: const EdgeInsets.symmetric(vertical: 13, horizontal: 14),
      enabledBorder:  OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFFDDE6F0), width: 1.2)),
      focusedBorder:  OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: kBlue, width: 1.8)),
      disabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: const Color(0xFFDDE6F0).withOpacity(0.5))),
    ),
    ...
    selectedItemBuilder: (ctx) => items.map((item) => Align(
      ...
      child: Text(display(item), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: kText), overflow: TextOverflow.ellipsis),
    )).toList(),
    items: items.map((item) => DropdownMenuItem<T>(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, mainAxisSize: MainAxisSize.min, children: [
        Text(display(item), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: kText), overflow: TextOverflow.ellipsis),
        Text(subtitle(item), style: const TextStyle(fontSize: 11, color: kSubtext), overflow: TextOverflow.ellipsis),
      ]),
    )).toList(),
  );
}
...
class _TField extends StatelessWidget {
  ...
  const _TField({required this.ctrl, required this.label, required this.icon, this.req = false, this.type});
  ...
  Widget build(BuildContext context) => TextFormField(
    ...
    decoration: InputDecoration(
      labelText: label, labelStyle: const TextStyle(fontSize: 12.5, color: kSubtext),
      ...
      focusedErrorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: kDanger, width: 1.8)),
    ),
  );
}
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(
          value != null
              ...
              : label,
          style: TextStyle(fontSize: 13, color: value != null ? kText : kSubtext),
        )),
      ]),
    ),
  );
...
class _Avatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: 38, height: 38,
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: List<Color>.from(_palettes[idx]),
            begin: Alignment.topLeft, end: Alignment.bottomRight),
...
class _TeamChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text(label,
        style: const TextStyle(fontSize: 10, color: kBlueMid, fontWeight: FontWeight.w600),
        overflow: TextOverflow.ellipsis),
  );
}
...
class _Badge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: color.withOpacity(0.09), borderRadius: BorderRadius.circular(100),
...
class _Empty extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(
        width: 50, height: 50,
        decoration: BoxDecoration(color: kBlueFaint, borderRadius: BorderRadius.circular(14)),
